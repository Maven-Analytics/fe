...
export const selectors = {
  ...
  getPaths: createSelector(
    [
      getPaths,
      courseSelectors.getCourses,
      enrollmentSelectors.getEnrollments,
      ...
      paths = paths.map(path => {
        ...
        path = path
          .set(
            'instructors',
            pathCourses
              .map(course => course.get('author'))
              .toSet()
              .toList()
          )
          .set(
            'length',
            pathCourses.reduce((hours, course) => hours + (course.get('length') || 0), 0)
          )
          .set('resumeUrl', pathCourses.getIn([0, 'url']));

        if (user) {
          const recommendedPath =
            user.get('recommended_paths') && user.get('recommended_paths').find(rp => rp.get('id') === path.get('id'));
          path = path.set('match', (recommendedPath && recommendedPath.get('percentage')) || 0);
        }
        ...
        const lastNotCompleteEnrollment =
          pathEnrollments && pathEnrollments.filter(enrollment => enrollment.get('percentage_completed') !== 1).first();
        const latestCourse = lastNotCompleteEnrollment
          ? pathCourses.find(course => course.get('thinkificCourseId') === lastNotCompleteEnrollment.get('course_id'))
          : path.getIn(['courses', 0]);
        const totalComplete = pathEnrollments.reduce(
          (total, enrollment) => enrollment.get('percentage_completed') + total,
          0
        );
...
function getPathEnrollments(path, enrollments) {
  return (
    ...
    path
      .get('courses')
      .map(course => enrollments.find(enrollment => enrollment.get('course_id') === course.get('thinkificCourseId')))
      .filter(e => e)
  );
}
